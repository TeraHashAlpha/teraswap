...
// [fix/token-sync-cron-landing] Single source of truth for "which chains does the pipeline
// guard" — the chain registry (the SAME source catalog-address-guard.test.ts's own
// registry-drift assertion uses). Previously a hand-maintained literal that silently fell
// behind the registry (missed 42161 for a full sprint) — deriving it means a chain added to
// the registry is audited here automatically, no second list to remember to update.
export const GUARD_CHAINS: readonly number[] = getSupportedChainIds()
 ...
 *
 * [fix/token-sync-cron-landing] MERGES per chain instead of overwriting the whole file: a
 * run for chain X replaces only chain X's rows, leaving every OTHER chain's rows (and their
 * exact JSON bytes) untouched. Before this, a single-chain run — which the per-chain cron
 * matrix now does — would silently wipe every other chain's cached verdicts, failing the
 * NEXT chain's `verdict-cache` check for a reason that has nothing to do with that chain.
 * `chains` names which chain(s) THIS run covers (rows for chains not in `verdicts` at all
 * are dropped from that set — a chain run with zero surviving tokens still "covers" itself).
 */
export function writeTrustFixture(
  ...
  const thisRunChains = new Set(chains)
  const newRows = sortVerdictsInPlace([...verdicts].filter((v) => thisRunChains.has(v.chainId)))

  let existingRows: Verdict[] = []
  let existingChains: number[] = []
  try {
    const parsed = JSON.parse(fs.readFileSync(file, 'utf8')) as { tokens?: Verdict[]; generatedFromChains?: number[] }
    existingRows = parsed.tokens ?? []
    existingChains = parsed.generatedFromChains ?? []
  } catch {
    // no existing fixture (first run, or a scratch path in a test) — nothing to preserve.
  }

  // Rows for a chain THIS run does not cover are carried over byte-for-byte (same key order
  // as read from disk, so JSON.stringify reproduces the exact prior bytes for that object).
  const keptRows = existingRows.filter((r) => !thisRunChains.has(r.chainId))
  const merged = sortVerdictsInPlace([...keptRows, ...newRows])
  const allChains = [...new Set([...existingChains.filter((c) => !thisRunChains.has(c)), ...chains])].sort(
    (a, b) => a - b,
  )
  ...
  const payload = {
    ...
    generatedFromChains: allChains,
    tokens: merged,
  }
  fs.writeFileSync(file, JSON.stringify(payload, null, 1) + '\n')
  return { file, count: merged.length }
}
