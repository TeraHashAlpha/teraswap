...
| [ADR-022](docs/ADR/ADR-022-r1-group-g-allowance-holder-recipient.md) | R1 Group G: unwrapping the recipient from `AllowanceHolder.exec` | Accepted (2026-09-03) | Closes the third gate ADR-021 deferred. New decode class: unwrap `exec`'s `data` argument, require the inner selector to be the Settler's `execute`, and validate `AllowedSlippage.recipient` (tuple field 0) with the same `isValidRecipient` as every other group. Also requires `target` AND `operator` to be whitelisted routers — `exec` with an arbitrary target is a transfer primitive. Restores the R1 ≡ SC-04 equality. Records that 0x is STILL not executable end-to-end: its `target` is the rotating Settler, which cannot be whitelisted. |
